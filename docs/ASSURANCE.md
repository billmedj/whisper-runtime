...
| An accepted result cannot overlap or regress an existing committed prefix; a new prefix cannot exceed its result; omission preserves the prior prefix. | `accepted_append_respects_committed_prefix`, `accepted_append_does_not_regress`, `accepted_claim_is_bounded_by_window`, `accepted_append_without_claim_preserves_prefix` | Committed-prefix, competing-commit, invalid-boundary, history-eviction, and no-finality compatibility tests in `tests/test_runtime.py` | Not yet recorded with a live audio stream or external finality policy. | The Lean module models one abstract append transition. It does not prove Python correspondence, timestamp alignment, or that a caller selected a correct finality boundary. |
...
| A transaction does not release capacity before registered backend work reaches its completion fence. | `capacity_released_iff`, `released_capacity_has_completion_or_recovery_basis`, `reachable_release_has_completion_or_recovery_basis` | Submission-gate, fence, cancellation, quarantine, and recovery tests in `tests/test_runtime.py`; fake-CUDA adapter tests check stream, event, commit, failure, and recovery ordering. | Two version-two Modal records observe a real T4 event fence before session publication and lease release on separate AWS and GCP workers. The version-six qualification repeats the release relation across five measured runtime runs, three cancellation runs, and eight injected-fault recovery runs on one AWS T4. | The Lean model starts at an already admitted execution and treats a successful fence as an observation. It does not model registration, CUDA, or the submission gate. The integration evidence covers one `tiny.en` FP32 single-lane case. Device memory is measured but not enforced. |
| Failure to prove backend quiescence retains the reservation. | `uncertain_observation_enters_quarantine`, `quarantined_result_is_withheld`, `quarantined_capacity_is_held`, `only_recovery_exits_quarantine` | Persistent-fence and quarantine-recovery tests in `tests/test_runtime.py`; the fake-CUDA adapter matrix covers cleanup, event creation, event record, and event synchronization failures. | The version-two records inject event-synchronization failures. The version-six qualification runs two repetitions at each of four harness points: cleanup, event creation, event record, and event synchronization. Every repetition injects two ordered failures, retains the reservation, blocks competing work, recovers, and commits one reuse run. | The formal recovery transition assumes that the implementation supplied valid recovery evidence. The injected integration faults are not physical CUDA driver failures. Recovery is in-process and backend-specific. |
...
revision on AWS and GCP T4 workers. One version-six qualification record adds
the registered cancellation and four-point harness fault matrix on an AWS T4.
They establish only the integration facts listed in this table and their closed
record schemas.
