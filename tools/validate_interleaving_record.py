"""Validate one native interleaving evidence record."""
...
from check_repository import ROOT, validate_native_interleaving_evidence
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate a native same-model interleaving evidence record."
    )
...
def validate_record(record: Any, location: str) -> list[str]:
    ...
    try:
        ...
        return [f"the native interleaving schema is invalid: {error}"]
...
def main() -> int:
    ...
    failures = validate_record(record, args.record.name)
    if failures:
