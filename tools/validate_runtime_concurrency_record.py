"""Validate one native runtime adapter concurrency evidence record."""
...
from check_repository import ROOT, validate_native_runtime_concurrency_evidence
...
def validate_runtime_concurrency_record(
    record: Any,
    location: str,
) -> list[str]:
    ...
    try:
        ...
        return [f"the native runtime concurrency schema is invalid: {error}"]
    ...
    if not schema_errors:
        failures.extend(validate_native_runtime_concurrency_evidence(record, location))
    return failures
...
def main() -> int:
    ...
    try:
        ...
        return 1
    failures = validate_runtime_concurrency_record(record, args.record.name)
    if failures:
